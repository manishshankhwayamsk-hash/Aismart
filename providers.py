...
async def generate_quiz_json(provider: str, key: str, extra: str | None, topic: str, level: str = "Class 6-10") -> dict:
    ...
    prompt = (
        ...
        f"Write the question BILINGUALLY: English sentence, then '/', then its Hindi "
        f"translation, e.g. 'What is X? / X kya hai?'. Write each option bilingually "
        f"the same way, e.g. 'Apple / Seb'. Respond with ONLY raw JSON, no markdown "
        f"fences, no extra text, in this exact shape: "
        f'{{"question": "...", "options": ["...", "...", "...", "..."], '
        f'"correct_index": 0}}. correct_index is the 0-based index of the correct option '
        f"in the options array. Keep the full bilingual question under 280 characters "
        f"and each bilingual option under 95 characters (Telegram poll limits)."
    )
...
async def generate_shayari(provider: str, key: str, extra: str | None, correct: bool, mention: str) -> str:
    ...
    if correct:
        mood = (
            ...
            "famous-movie-dialogue-style victory line, for winning/getting it "
            "right, Instagram-caption style, lighthearted — pick whichever fits"
        )
    else:
        mood = (
            "a funny reaction for getting the answer wrong — randomly pick ONE "
            "style: (a) a dramatic mock-'sad and alone' 2-line Hinglish shayari, "
            "(b) a famous funny Bollywood/movie dialogue twisted to fit the "
            "moment, or (c) an exaggerated 'akela/lonely' comic one-liner — in "
            "the over-the-top comic style of Instagram reels captions. Funny "
            "and dramatic, never genuinely sad or mean"
        )
    prompt = (
...
async def generate_reply(provider: str, key: str, extra: str | None, user_message: str) -> str:
    prompt = (
        "You are KRX AI Smart, a helpful, friendly Telegram assistant. Answer "
        "whatever the user asks, on any topic, clearly and naturally, the way "
        "a knowledgeable friend would. Reply in the same language style the "
        "user wrote in (Hindi, English, or Hinglish). Keep it conversational, "
        "not robotic. Here is their message:\n\n"
        f"{user_message}"
    )
    return await generate_text(provider, key, extra, prompt)
