...
async def init_db():
    ...
    async with _pool.acquire() as con:
        ...
        # seed default topics only if table is empty (kept from previous bot theme)
        count = await con.fetchval("SELECT COUNT(*) FROM topics")
        if count == 0:
            defaults = [
                "Network Security", "Cryptography", "Web Security",
                "Malware & Reverse Engineering", "Social Engineering",
                "Ethical Hacking", "OWASP Top 10", "Linux & CLI Security",
                "Cloud Security", "Wireless Security", "Digital Forensics",
